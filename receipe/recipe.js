function renderStars(rating) {
  ...
  return `<span class="rating" role="img" aria-label="${ariaLabel}">${stars}</span>`;
}
...
function renderCards(list) {
  ...
  main.innerHTML = '';
  if (list.length === 0) {
    main.innerHTML = '<p style="color:#888;text-align:center;padding:2rem;">No recipes found.</p>';
    return;
  }
  list.forEach(recipe => {
    const tag = recipe.tags[0] ? recipe.tags[0].toLowerCase() : '';
    main.innerHTML += `
      <article class="recipe-card">
        ...
        <div class="card-info">
          <span class="tag tag-inline">${tag}</span>
